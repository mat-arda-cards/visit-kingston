...
| Was | Now | Where |
|---|---|---|
| P0 — migrate file stores off the local disk | **Done.** Dual-backend seam: every store branches on env. Neon Postgres (`overlay`/`analytics_event`/`survey_response`/`ferry_observation` tables), Vercel Blob for images, Upstash Redis for shared rate-limit. Stores unchanged above the seam. | `src/lib/db.ts`, `src/lib/blob-store.ts`, `src/lib/rate-limit.ts` (since E05: Postgres is required — the env branch and `src/lib/db.ts` are gone; schema `src/lib/db/schema.ts` + `db/migrations/`; the legacy data-move script is replaced by `npm run import:data-dir`, `scripts/import-data-dir.ts`) |
| P0 — production deploy | **Done (Phase 1 live on Render).** Docker/standalone, Starter web + 1 GB disk at `/data`, `/api/health` 503-gates on `dataWritable`. Auto-deploy on push. *(Since E05 the gate also requires `dbOk` — Neon Postgres is the required home for structured data; the disk keeps images/photos.)* | `Dockerfile`, `render.yaml`, `fly.toml`, `src/app/api/health/route.ts`, [DEPLOY.md](DEPLOY.md) |
...
| P0 — analytics DB-backed store | **Done.** `saveEvent`/`summarize` → `analytics_event`. *(E05: DB-only — the old `hasDb()` file/DB branch is gone.)* (Debt survives — see [§6](#6-analytics-v2).) | `src/lib/analytics-store.ts` |
