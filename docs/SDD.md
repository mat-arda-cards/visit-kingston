...
- **`/api/portal/needs`** — `{action:"slots", id, delta:±1}` clamps `slotsFilled` to `0..slotsTotal`; bare `YYYY-MM-DD` anchored at Pacific midnight. The slots tick on a **live** shift is the one member write exempt from the E08 moderation hold (documented exception 2026-07-20; live-only — pending updates in place, hidden/draft hold for review).
...
- **`/api/track`** — reads raw text (sendBeacon posts `text/plain`), always returns `{ok:true}`; drops `/admin` paths; enforces the geo-ping privacy box (lat 47.5–48.1, lng −123.0–−122.2), rounds and classifies into a named area, then **stores only the area bucket — coordinates are inspected transiently and never persisted** (E11); outbound taps to food/health-assistance destinations and events on sensitive paths are dropped entirely (`src/lib/privacy/policy.ts`); accepts a `consent` event type (notice version only, no location); IP peeked only to label loopback/RFC-1918 as `dev-local` or for the in-memory GeoLite2 lookup, never stored.
